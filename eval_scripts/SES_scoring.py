...
args.add_argument('--result_path', type=str, default='results/streaming_event_summary', description='Path to the result files')
args.add_argument('--output_path', type=str, default='scores/streaming_event_summary', description='Path to the output files')
...
prompt = '''
    请仔细阅读话题事件摘要参考答案<place_holder>，并根据参考答案对给出的话题事件摘要内容进行评分，评分范围为 0 到 5 分，评分维度包括：
    - 准确性（Accuracy）：是否准确反映参考答案中的时间、事件和因果关系；
    - 完整性（Completeness）：是否完整地涵盖了参考答案中的关键时间点和重要事件；
    请从两个评分维度为事件的摘要内容打分，并输出综合评判后的评分，
    准确性评分标准定义如下：
    ...
    完整性评分标准定义如下：
    - 5 分：完全符合参考答案内容，涵盖了所有关键时间点和重要事件；
    - 4 分：基本符合参考答案内容，遗漏了少量次要信息；
    - 3 分：部分符合参考答案内容，遗漏了一些重要事件或时间点；
    - 2 分：少部分符合参考答案内容，遗漏较多关键信息；
    - 1 分：勉强符合参考答案内容，几乎未涵盖关键信息；
    - 0 分：完全不符合参考答案内容，信息缺失严重或空白；
    请为每个维度分别打分。
    输出格式要求：
    - 如果两个维度的评分为：准确性：2，完整性：2，则输出：准确性：2，完整性：2
...
ground_truth = json.load(open("datasets/streaming_event_summary/ground_truth.json"))
pattern = r"准确性：\d+，完整性：\d+"
