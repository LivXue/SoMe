...
from tasks import user_comment_silmulation as task
...
parser.add_argument('--output_path', type=str, default="results/user_comment_silmulation", description="The output path for the results")
...
vllm_config = {
    'model_server': args.base_url, #'https://api.moonshot.cn/v1',
...
bot = SocialMediaAgent(system_message=task.zh_prompt, 
                       ...
                       llm=vllm_config)
comment_data = json.load(open("datasets/user_comment_silmulation/ground_truth.json", encoding='utf-8'))
post_data = json.load(open("database/post_data/all_posts.json", encoding='utf-8'))
for user_id in tqdm(comment_data):
    if user_id not in results:
        results[user_id] = {}
    for i, entry in enumerate(comment_data[user_id]):
        ...
        for retry in range(1):
            ...
            try:
                ...
                messages = []
                query = f"请判断帖子```{json.dumps(post, ensure_ascii=False)}```下的评论“{comment}”是否是由ID为{user_id}的用户发表。"
